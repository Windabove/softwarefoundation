...
Proof.
  ...
  assert (n + m = m + n) as H. { rewrite add_comm. reflexivity. }
...
(** **** Exercise: 3 stars, standard, optional (more_exercises)
    ...
    _think_ about whether (a) it can bereflexivity. proved using only
...
Proof.
  intros n. simpl. rewrite add_comm. reflexivity.
Qed.
...
Proof.
  intros n m p. induction n as [| n' IHn'].
  - rewrite mult_0_plus'. simpl. reflexivity.
...
Theorem hom_bin_nat : forall m : bin,
  bin_to_nat (incr m) = S (bin_to_nat m).
Proof.
  intros m. induction m as [| m0 | m1 IHm1].
  - reflexivity.
  - simpl. reflexivity.
  - simpl. rewrite IHm1.
...
Proof.
  intros n. induction n as [| n' IHn'].
  - reflexivity.
  - simpl. rewrite hom_bin_nat. rewrite IHn'. reflexivity.
Qed.
...
Fixpoint normalize (n : bin) :=
  match n with
    | Z => Z
    | B1 n' => B1 (normalize n')
    | B0 n' => let N := normalize n' in match N with Z => Z | _ => B0 N end
  end.

Theorem normalize_B0_comm (n : bin) : (n <> Z) -> n = normalize n -> B0 n = normalize (B0 n).
Proof.
  intros HZ H.
  simpl.
  destruct (normalize n) eqn:E.
  - apply HZ in H. destruct H.
  - rewrite H. reflexivity.
  - rewrite H. reflexivity.
Qed.

Theorem normalize_inv (n : bin) : normalize (normalize n) = normalize n.
Proof.
  induction n.
  - reflexivity.
  - simpl.
    ...
    + reflexivity.
    + symmetry. apply normalize_B0_comm. discriminate. auto.
    + symmetry. apply normalize_B0_comm. discriminate. auto.
  - simpl. rewrite IHn. reflexivity.
Qed.

Theorem normalize_incr_comm (n : bin) : incr (normalize n) = normalize (incr n).
Proof.
  induction n.
  - reflexivity.
  - simpl.
...
Theorem nat_to_bin_double (n : nat) : nat_to_bin (n + n) = normalize (B0 (nat_to_bin n)).
Proof.
  induction n.
  - reflexivity.
...
Theorem normalization (n : bin) : normalize n = nat_to_bin (bin_to_nat n).
Proof.
  induction n as [| n' | n'].
  - reflexivity.
  - simpl. 
    ...
    destruct (normalize n') eqn:E.
    + reflexivity.
    + apply normalize_B0_comm.
      discriminate.
      rewrite <- E.
      rewrite normalize_inv.
      reflexivity.
    + simpl.
      apply normalize_B0_comm.
      discriminate.
