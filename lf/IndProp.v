...
Proof.
  intros.
  rewrite <- ev'_ev in H.
  rewrite <- ev'_ev in H0.
  assert (H1 : ev' (double n + (m + p))).
  {
    rewrite double_plus.
    rewrite PeanoNat.Nat.add_shuffle1.
    apply (ev'_sum (n + m) (n + p) H H0).
  }
  apply ev_ev__ev with (n:=double n).
  - rewrite ev'_ev in H1. apply H1.
  - apply ev_double.
Qed.
...
Inductive subseq : list nat -> list nat -> Prop :=
  | s1 l: subseq [] l
  | s2 x l1 l2 (H : subseq l1 l2) : subseq l1 (x :: l2)
  | s3 x l1 l2 (H : subseq l1 l2) : subseq (x :: l1) (x :: l2)
.
...
Proof.
  ...
  - intros. simpl.
    inversion H.
    + apply s1.
    + apply IHl2 with (l3:=l3) in H2.
      apply s2.
      apply H2.
    + apply s3.
      apply IHl2 with (l1:=l0).
      apply H2.
Qed.

Lemma subseq_sub : forall x (l1 l2: list nat),
...
Proof.
  ...
  - intros. induction H.
    + apply s1.
    + inversion H0.
      ++ apply subseq_sub in H3. apply s2. apply (IHl3 _ _ H H3).
      ++ apply s2. apply (IHl3 _ _ H H2).
    + inversion H0.
      ++ apply s2. apply s3 with (x:=x0) in H. apply (IHl3 _ _ H H3).
      ++ apply s3. apply (IHl3 _ _ H H2).
Qed.
...
(** [R 6 [3;2;1;0]] with its negation is unprovable. **)
